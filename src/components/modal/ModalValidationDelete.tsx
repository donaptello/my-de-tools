...
interface Props {
  ...
  monitoring?: MonitoringConfigurationData;
  darkMode: boolean;
  showDeleteConfirm: boolean;
  setShowDeleteConfirm: (value: boolean) => void;
  onConfirm?: (conn: ConnectionData | undefined) => void;
  onConfirmMonitoring?: (monn: MonitoringConfigurationData | undefined) => void;
}
      ...
      {showDeleteConfirm && connection ? (
        ...
        >
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            >
              <h3 className="text-lg font-semibold mb-2">Delete Table Params Row Count</h3>

              <p className="text-sm text-gray-500 mb-4">
                Are you sure you want to delete{" "}
                <span className="font-medium">{monitoring?.schemas}.{monitoring?.tableNameSource}</span>? <br/> This
                action cannot be undone.
              </p>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  onClick={() => {
                    setShowDeleteConfirm(false);
                    onConfirmMonitoring?.(monitoring);
                  }}
                  className="rounded-md bg-red-600 px-4 py-2 text-sm text-white hover:bg-red-500"
                >
                  Delete

...
import { MonitoringTableParams, MonitoringTotalDataRes, MonitoringTotalTableRes } from "../types/Monitoring.types";
...
export function useMonitoringTable() {
    ...
    useEffect(() => {
        setLoading(true);
        monitoringService
        .getTableData(query).then(res => {setData(res)})
...
export function useMonitoringData() {
    ...
    useEffect(() => {
        setLoading(true);
        monitoringService
        .getTotalData().then(res => {setData(res)})
        .finally(() => setLoading(false));
    }, [])
